...
LCMETERINIT:	MOV	A,#040h				;D6 LC Meter	0=L, 1=C
		...
		;Calculate LCCB=((1/2*Pi*F1)^2)*LCCA*(1/Ccal)
		MOV	DPTR,#FPTWO
		LCALL	PUSHC				;PUSH ARG IN DPTR TO STACK
		MOV	DPTR,#FPPI
		LCALL	PUSHC				;PUSH ARG IN DPTR TO STACK
...
;IN:	Nothing
;OUT:	Nothing
LMETER:		MOV	A,#000h				;D6 LC Meter	0=L, 1=C
							;D7 LC Meter	0=F1, 1=F2 (Adding C Cal)
		MOV	R1,#LOW LCF3
		MOV	R3,#HIGH LCF3
		...
		MOV	R3,#HIGH LCF3
		ACALL	LCCALC
		;Calculate ((F1/F3)^2)-1)*((F1/F2)^2)-1)*((1/(2*Pi*F1))^2)*(1/Ccal)
		MOV	R0,#LOW LCCB
		MOV	R2,#HIGH LCCB
		LCALL	PUSHAS				;PUSH R2:R0 TO ARGUMENT
		LCALL	FLOATING_MUL
...
CMETER:		MOV	A,#040h				;D6 LC Meter	0=L, 1=C
		...
		MOV	R3,#HIGH LCF3
		ACALL	LCCALC
		;Calculate ((F1/F3)^2)-1)/((F1/F2)^2)-1)
		MOV	R0,#LOW LCCA
		MOV	R2,#HIGH LCCA
		LCALL	PUSHAS				;PUSH R2:R0 TO ARGUMENT
		LCALL	FLOATING_DIV
		;Calculate ((F1/F3)^2)-1)/((F1/F2)^2)-1)*Ccal
		MOV	DPTR,#FPCCAL
		LCALL	PUSHC				;PUSH ARG IN DPTR TO STACK
		LCALL	FLOATING_MUL
		LCALL	PRINTFP
...
FPCCAL:		DB 77h,00h,00h,00h,00h,10h		;1nF=1e-9
;------------------------------------------------------------------
