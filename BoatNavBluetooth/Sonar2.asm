...
Resize_Image proc uses ebx esi edi,hBmp:HBITMAP,wt:DWORD,ht:DWORD
	...
	invoke GdipDrawImageRectI,gfx,image1,0,0,iwt,ht
	invoke GdipDisposeImage,image1
...
UpdateBitmap proc uses ebx esi edi,NewRange:DWORD
	...
	mov		fset,FALSE
	mov		ebx,MAXXECHO
	add		ebx,sonardata.sonarofs
	mov		edi,sonardata.sonarbmpinx
	.while sdword ptr ebx>=0
		mov		eax,edi
		mov		edx,sizeof SONARBMP
		mul		edx
		mov		esi,eax
		.break .if !sonardata.sonarbmp.hBmp[esi]
		sub		ebx,sonardata.sonarbmp.wt[esi]
		.if sdword ptr ebx<=MAXXECHO
			.if !fset
				mov		sonardata.cursonarbmpinx,edi
				mov		fset,TRUE
			.endif
			invoke GetRangePtr,sonardata.sonarbmp.RangeInx[esi]
			mov		ecx,sonardata.sonarrange.range[eax]
			mov		eax,MAXYECHO
			mul		ecx
			mov		ecx,NewRange
			div		ecx
			mov		edx,sonardata.sonarbmp.wt[esi]
			invoke Resize_Image,sonardata.sonarbmp.hBmp[esi],edx,eax
		...
		.endif
		dec		edi
		and		edi,MAXSONARBMP-1
		.break .if edi==sonardata.sonarbmpinx
	.endw
	invoke DeleteDC,mDC
	ret
...
Update:
	...
	.if nUpdate==1
		...
		.if sonardata.sonarbmp.wt[esi]==MAXSONARBMPWT || eax!=sonardata.sonarbmp.RangeInx[esi]
			inc		sonardata.sonarbmpinx
			and		sonardata.sonarbmpinx,MAXSONARBMP-1
			call	GetBmpPtr
			movzx	eax,sonardata.STM32Echo
			mov		sonardata.sonarbmp.RangeInx[esi],eax
			mov		sonardata.sonarbmp.wt[esi],1
			invoke wsprintf,addr sonardata.options.text,addr szFmtDec,sonardata.RangeVal
		.else
			inc		sonardata.sonarbmp.wt[esi]
		.endif
		.if !sonardata.sonarbmp.hBmp[esi]
			invoke CreateCompatibleBitmap,hDC,MAXSONARBMPWT,MAXYECHO
			mov		sonardata.sonarbmp.hBmp[esi],eax
		.endif
		mov		eax,mapdata.iLon
		mov		sonardata.sonarbmp.iLon[esi],eax
		mov		eax,mapdata.iLat
		mov		sonardata.sonarbmp.iLat[esi],eax
		;Draw echo
		invoke SelectObject,mDC,sonardata.sonarbmp.hBmp[esi]
		push	eax
		mov		ebx,1
		mov		edi,sonardata.sonarbmp.wt[esi]
		mov		rect.right,edi
		dec		edi
		mov		rect.left,edi
		mov		rect.top,0
		...
		.while ebx<MAXYECHO
			...
			.if eax
				...
				.endif
				invoke SetPixel,mDC,edi,ebx,eax
			.endif
			lea		ebx,[ebx+1]
		.endw
		...
		.else
			.if sonardata.hReplay
				...
				.if sonarreplay.Version>=200
					...
					.if dwread==sizeof SONARREPLAY
						...
						.endif
						invoke ReadFile,sonardata.hReplay,addr sonardata.STM32Echo,MAXYECHO,addr dwread,NULL
						mov		eax,mapdata.iLon
						mov		iLon,eax
						mov		eax,mapdata.iLat
						mov		iLat,eax
				...
				.else
					...
					mov		sonardata.fReplayRead,FALSE
					movzx	eax,sonardata.STM32Echo
					.if al!=sonardata.STM32Echo[MAXYECHO]
						mov		sonardata.dptinx,0
						mov		rngchanged,4
					.endif
					invoke SetRange,eax
			...
			.elseif sonardata.fBluetooth
				.if fdataready
					...
					rep movsb
					mov		edi,offset sonardata.STM32Echo
					mov		ecx,MAXYECHO
					rep movsb
			...
			.else
				...
				.while edx<edi
					invoke Random,50
					add		eax,255-50
					mov		sonardata.STM32Echo[edx],al
					inc		edx
				.endw
				...
				.if eax>100 && eax<MAXYECHO-1
					...
					.if eax>124 && eax<130
						;Random fish
						mov		ah,al
						mov		word ptr sonardata.STM32Echo[edx],ax
						mov		word ptr sonardata.STM32Echo[edx+MAXYECHO],ax
						mov		word ptr sonardata.STM32Echo[edx+MAXYECHO*2],ax
					.endif
				.endif
...
ShowEcho:
	.if sonardata.hLog
		...
		invoke WriteFile,sonardata.hLog,addr sonardata.STM32Echo,MAXYECHO,addr dwwrite,NULL
	.endif
	movzx	eax,sonardata.STM32Echo
	.if al!=sonardata.STM32Echo[MAXYECHO]
		call	CopyEcho
...
MoveEcho:
	;Move echo arrays
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*3],addr sonardata.STM32Echo[MAXYECHO*2],MAXYECHO
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*2],addr sonardata.STM32Echo[MAXYECHO*1],MAXYECHO
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*1],addr sonardata.STM32Echo[MAXYECHO*0],MAXYECHO
	retn

CopyEcho:
	;Copy echo arrays
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*3],addr sonardata.STM32Echo,MAXYECHO
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*2],addr sonardata.STM32Echo,MAXYECHO
	invoke RtlMoveMemory,addr sonardata.STM32Echo[MAXYECHO*1],addr sonardata.STM32Echo,MAXYECHO
	retn
...
FindDepth:
	...
	.while ebx<MAXYECHO/2
		xor		ch,ch
		mov		ax,word ptr sonardata.STM32Echo[ebx+MAXYECHO*0]
		mov		dx,word ptr sonardata.STM32Echo[ebx+MAXYECHO*0+2]
		.if al<cl && ah<cl && dl<cl && dh<cl
			inc		ch
		.endif
		mov		ax,word ptr sonardata.STM32Echo[ebx+MAXYECHO*1]
		mov		dx,word ptr sonardata.STM32Echo[ebx+MAXYECHO*1+2]
		.if al<cl && ah<cl && dl<cl && dh<cl
			inc		ch
		.endif
		mov		ax,word ptr sonardata.STM32Echo[ebx+MAXYECHO*2]
		mov		dx,word ptr sonardata.STM32Echo[ebx+MAXYECHO*2+2]
		.if al<cl && ah<cl && dl<cl && dh<cl
			inc		ch
		.endif
		mov		ax,word ptr sonardata.STM32Echo[ebx+MAXYECHO*3]
		mov		dx,word ptr sonardata.STM32Echo[ebx+MAXYECHO*3+2]
		.if al<cl && ah<cl && dl<cl && dh<cl
			inc		ch
		.endif
		.break .if ch==4
	...
	.if ch==4
		...
		.while ebx<MAXYECHO
			...
			.while ecx<16
				...
				movzx	eax,sonardata.STM32Echo[ebx+ecx+MAXYECHO*0]
				add		edx,eax
				movzx	eax,sonardata.STM32Echo[ebx+ecx+MAXYECHO*1]
				add		edx,eax
				movzx	eax,sonardata.STM32Echo[ebx+ecx+MAXYECHO*2]
				add		edx,eax
				movzx	eax,sonardata.STM32Echo[ebx+ecx+MAXYECHO*3]
				add		edx,eax
				inc		ecx
			.endw
...
FindFish:
	.if sonardata.FishDetect || sonardata.FishAlarm
		...
		.elseif edi>sonardata.minyecho
			...
			.while edi>ebx
				dec		edi
				mov		ax,word ptr sonardata.STM32Echo[edi]
				mov		dx,word ptr sonardata.STM32Echo[edi+MAXYECHO]
				.if al<cl && ah<cl && dl<cl && dh<cl
					inc		ch
				.else
					xor		ch,ch
		...
		.endif
		.while ebx<edi
			mov		ax,word ptr sonardata.STM32Echo[ebx]
			;2x3
			mov		dx,word ptr sonardata.STM32Echo[ebx+MAXYECHO]
			mov		cx,word ptr sonardata.STM32Echo[ebx+MAXYECHO*2]
			...
			.if al>=SMALLFISHECHO && ah>=SMALLFISHECHO && dl>=SMALLFISHECHO && dh>=SMALLFISHECHO && cl>=SMALLFISHECHO && ch>=SMALLFISHECHO
				.if sonardata.FishDetect
					...
					call	CheckFish
					.if eax
						movzx	edx,sonardata.STM32Echo[ebx]
...
Show0:
	...
	.if eax==1
		;1*2
		.while esi<MAXYECHO
			mov		al,sonardata.STM32Echo[esi]
			mov		ah,sonardata.STM32Echo[esi+MAXYECHO]
			.if (al<bl || ah<bl)
				mov		al,0
			.endif
	...
	.elseif eax==2
		...
		.while esi<MAXYECHO-1
			mov		ax,word ptr sonardata.STM32Echo[esi]
			mov		dx,word ptr sonardata.STM32Echo[esi+MAXYECHO]
	...
	.elseif eax==3
		...
		.while esi<MAXYECHO-1
			mov		ax,word ptr sonardata.STM32Echo[esi]
			mov		dx,word ptr sonardata.STM32Echo[esi+MAXYECHO]
			mov		cx,word ptr sonardata.STM32Echo[esi+MAXYECHO*2]
...
Show25:
	...
	.else
		.while esi<MAXYECHO
			;Blend in 25% of previous echo
			movzx	eax,sonardata.STM32Echo[esi]
			shl		eax,2
			movzx	edx,sonardata.STM32Echo[esi+MAXYECHO]
			add		eax,edx
			mov		ecx,5
			xor		edx,edx
			div		ecx
...
Show33:
	...
	.else
		.while esi<MAXYECHO
			;Blend in 33% of previous echo
			movzx	eax,sonardata.STM32Echo[esi+MAXYECHO]
			mov		ecx,3
			xor		edx,edx
			div		ecx
			mov		edx,eax
...
Show75:
	...
	.else
		.while esi<MAXYECHO
			...
			movzx	eax,sonardata.STM32Echo[esi+MAXYECHO]
			shl		eax,2
			movzx	edx,sonardata.STM32Echo[esi]
			add		eax,edx
			mov		ecx,5
			xor		edx,edx
			div		ecx
...
SonarClear proc uses ebx esi
	...
	.while ebx
		.if [esi].SONARBMP.hBmp
			...
			mov		[esi].SONARBMP.RangeInx,0
			mov		[esi].SONARBMP.wt,0
		.endif
...
SonarChildProc proc uses ebx esi edi,hWin:HWND,uMsg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	.elseif eax==WM_LBUTTONDOWN
		.if mapdata.fUnLocked
			...
			movsx	edx,dx
			mov		mousept.y,edx
			mov		eax,sonardata.sonarofs
			mov		sonardata.cursonarofs,eax
		.endif
	...
	.elseif eax==WM_MOUSEMOVE
		...
		.if eax==hWin
			...
			mov		pt.y,edx
			sub		eax,mousept.x
			add		eax,sonardata.cursonarofs
			.if sdword ptr eax<0
				xor		eax,eax
			.endif
			mov		sonardata.sonarofs,eax
		.endif
