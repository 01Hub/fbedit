...
SizeingRect proc uses esi,hWin:HWND,fLocked:DWORD
	...
	mov		ParPt.x,0
	mov		ParPt.y,0
	invoke ClientToScreen,hInvisible,addr ParPt
	invoke GetWindowRect,hWin,addr rect
...
;		invoke NotifyParent
	.elseif eax==WM_INITDIALOG
		invoke SetWindowPos,hWin,0,10,10,0,0,SWP_NOSIZE or SWP_NOZORDER
;		mov		eax,hWin
;		mov		hDlg,eax
;		.if hDlgMnu
;			invoke GetMenu,hWin
...
MakeDialog proc uses esi edi ebx,hWin:HWND,hMem:DWORD
	LOCAL	nInx:DWORD
...
;		mov		eax,[edi].DIALOG.id
	...
	.endif
	pop		ebx
	invoke CreateDialogIndirectParam,hInstance,ebx,hWin,offset MakeDlgProc,0
	push	eax
