...
SendKeys proc uses ebx esi,lpki:DWORD,nSleep:DWORD

	mov		esi,lpki
	xor		ebx,ebx
	.while ebx<2
		invoke SendInput,1,esi,sizeof INPUTKEY
		invoke Sleep,30
		inc		ebx
		lea		esi,[esi+sizeof INPUTKEY]
	.endw
	invoke Sleep,nSleep
	ret
...
WndProc proc hWin:HWND,uMsg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	.elseif eax==WM_COMMAND
		...
		.endif
	.elseif eax==WM_NCRBUTTONUP
		mov		esi,offset szHome
		.while byte ptr [esi]
			movzx	eax,byte ptr [esi]
			mov		keysend.wScan,ax
			mov		keysend.wScan[sizeof INPUTKEY],ax
			invoke SendKeys,addr keysend,30
			inc		esi
		.endw
	.elseif eax==WM_SIZE
