...
public class STM32_SCP {
    ...
    public byte PixDiv = ScopeMain.WAVEGRID;
    public byte nDiv = ScopeMain.WAVEGRIDX;
    public byte Mag = 0;
        ...
        try {
            if (BlueTooth.btconnected) {
                ...
                if (BlueTooth.BTGetBytes(8)) {
                    ...
                    s = ScopeMain.FormatFrequency(scpfrq);
                }
                BlueTooth.btbusy = true;
                ...
                n = (ScopeMain.WAVEXSIZE * 2 * 3) / 4;
                if (BlueTooth.BTGetBytes(n)) {
                    i = 0;
                    j = 0;
                    while (i < ScopeMain.WAVEXSIZE) {
                        f = BTToInt(j) & 0xFFFFFF;
                        g = (f >> 12) & 0xFFF;
                        f &= 0xFFF;
                        ScopeMain.scpWave[i] = (short)f;
                        i++;
                        ScopeMain.scpWave[i] = (short)g;
                        i++;
                        j += 3;
                    }
                }
