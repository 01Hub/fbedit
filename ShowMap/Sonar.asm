	...
	.else
		...
		.elseif sonardata.fSTLink && sonardata.fSTLink!=IDIGNORE
			...
			.if !(status & 255)
				...
				mov		ebx,eax
				;Setup gain array
				mov		edi,offset sonardata.GainArray
				xor		ecx,ecx
				.if sonardata.AutoGain
					.while ecx<MAXYECHO
						mov		eax,sonarrange.gain[ebx+ecx*DWORD]
...
PrintDec eax
				...
				.else
					...
					.while ecx<MAXYECHO
						mov		sonardata.GainArray[edi],ax
						lea		edi,[edi+WORD]
						inc		ecx
					.endw
				.endif
	...
	.if sonardata.NoiseReject
		.while ebx<MAXYECHO
			...
			.if al<sonardata.Noise || ah<sonardata.Noise
				mov		al,0
			.endif
			mov		sonardata.EchoArray[ebx],al
			inc		ebx
		.endw
	.else
		.while ebx<MAXYECHO
...
LoadSonarFromIni proc uses ebx esi edi
	...
	.while ebx<32
		...
		.while esi<MAXYECHO
			invoke GetItemInt,addr buffer,0
			mov		sonarrange.gain[edi+esi*DWORD+15*DWORD],eax
			lea		esi,[esi+16]
		.endw
	...
	.while ebx<sonardata.MaxRange
		...
		xor		ecx,ecx
		.while esi<MAXYECHO
			mov		eax,sonarrange.gain[edi+esi*DWORD+15*DWORD]
			push	eax
			sub		eax,ecx
			mov		tmp,eax
			fild	tmp
			...
			.while ebx<15
				...
				.endif
				lea		edx,[esi+ebx]
				mov		sonarrange.gain[edi+edx*DWORD],eax
				inc		ebx
			.endw
			pop		ebx
			pop		ecx
		...
		inc		ebx
	.endw
;	mov		edi,sizeof RANGE*0
;	xor		esi,esi
;	.while esi<MAXYECHO
;		mov		eax,sonarrange.gain[edi+esi*DWORD]
;		PrintDec eax
;		inc		esi
;	.endw
	ret
