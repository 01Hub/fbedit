...
;Clear 32K ram
CLS:
		MOV	DPTR,#MSGCLS	;Set ADP to 0000h and init auto write mode
		ACALL	SENDCOMMANDS
		MOV	R7,#80h
		MOV	R6,#00h
CLS1:
...
SETCGRAM:
		MOV	DPTR,#MSGCGRAM	;Set ADP to 2400h and init auto write mode
		ACALL	SENDCOMMANDS
		MOV	DPTR,#CGRAM
...
PRINTLINE:
		...
		MOV	R6,A
		MOV	A,B
		ADDC	A,#10h		;Add start of text address
		MOV	R7,A
		MOV	A,R6		;ADP LSB
		ACALL	WRITEDATA
		MOV	A,R7		;ADP MSB
...
MSGCGRAM:
		DB	00h,24h,24h	;CG RAM address pointer set, character code 80h to 85h
		DB	00h,00h,0B0h	;auto write mode
		DB	0FFh
